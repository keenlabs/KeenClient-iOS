...
- (void)testAddEvent {
    ...
    XCTAssertFalse([client addEvent:[NSDictionary dictionary] toEventCollection:nil error:&error],
                   @"addEvent should fail");
    XCTAssertNotNil(error, @"nil collection should return NO");
    error = nil;

    XCTAssertFalse([clientI addEvent:[NSDictionary dictionary] toEventCollection:nil error:&error],
                   @"addEvent should fail");
    XCTAssertNotNil(error, @"nil collection should return NO");
    error = nil;
    ...
    XCTAssertNotNil([[error userInfo] objectForKey:NSUnderlyingErrorKey],
                    @"and event that can't be serialized should return the underlaying error");
    error = nil;
...
- (void)testEventWithDictionary {
    ...
    NSDictionary *eventDictionary =
        [NSJSONSerialization JSONObjectWithData:[json dataUsingEncoding:NSUTF8StringEncoding] options:0 error:nil];
...
- (void)testUploadWithNoEvents {
    ...
    [self waitForExpectationsWithTimeout:kTestExpectationTimeoutInterval
                                 handler:^(NSError *_Nullable error) {
                                     XCTAssertEqual([KIODBStore.sharedInstance
                                                        getTotalEventCountWithProjectID:[mock config].projectID],
                                                    0,
...
- (void)testDeleteAfterMaxAttempts {
    ...
    [self waitForExpectationsWithTimeout:kTestExpectationTimeoutInterval
                                 handler:^(NSError *_Nullable error) {
                                     ...
                                     XCTAssertTrue([[KIODBStore.sharedInstance
                                                        getEventsWithMaxAttempts:3
                                                                    andProjectID:[mock config].projectID] allKeys]
                                                           .count == 0,
                                                   @"There should be no files after 3 unsuccessfull attempts.");
                                 }];
}
...
- (void)testIncrementEvenOnNoResponse {
    ...
    [self
        waitForExpectationsWithTimeout:kTestExpectationTimeoutInterval
                               handler:^(NSError *_Nullable error) {
                                   ...
                                   XCTAssertTrue([[KIODBStore.sharedInstance
                                                      getEventsWithMaxAttempts:3
                                                                  andProjectID:[mock config].projectID] allKeys]
                                                         .count == 0,
                                                 @"There should be no events with less than 3 unsuccessful attempts.");
                                   XCTAssertTrue([[KIODBStore.sharedInstance
                                                      getEventsWithMaxAttempts:4
                                                                  andProjectID:[mock config].projectID] allKeys]
                                                         .count == 1,
                                                 @"There should be one event with less than 4 unsuccessful attempts.");
                               }];
}
...
- (void)testUploadSkippedNoNetwork {
    XCTestExpectation *uploadFinishedBlockCalled =
        [self expectationWithDescription:@"Upload finished block should be called."];
...
- (void)testUploadMultipleEventsDifferentCollectionsOneFailsForServerReason {
    ...
    [self waitForExpectationsWithTimeout:kTestExpectationTimeoutInterval
                                 handler:^(NSError *_Nullable error) {
                                     ...
                                     XCTAssertTrue([KIODBStore.sharedInstance
                                                       getTotalEventCountWithProjectID:[mock config].projectID] == 1,
                                                   @"There should be 1 events after a partial upload.");
                                 }];
}
