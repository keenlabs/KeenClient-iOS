...
+ (NSString *)descriptionForCode:(HTTPCode)code {
    ...
    switch (code) {
        ...
        case HTTPCode100Continue:
            description = @"This means that the server has received the request headers, and that the client should "
                          @"proceed to send the request body (in the case of a request for which a body needs to be "
                          @"sent; for example, a POST request). If the request body is large, sending it to a server "
                          @"when a request has already been rejected based upon inappropriate headers is inefficient. "
                          @"To have a server check if the request could be accepted based on the request's headers "
                          @"alone, a client must send Expect: 100-continue as a header in its initial request and "
                          @"check if a 100 Continue status code is received in response before continuing (or receive "
                          @"417 Expectation Failed and not continue).";
            break;

        case HTTPCode101SwitchingProtocols:
            description = @"This means the requester has asked the server to switch protocols and the server is "
                          @"acknowledging that it will do so.";
            break;

        case HTTPCode102Processing:
            description = @"As a WebDAV request may contain many sub-requests involving file operations, it may take a "
                          @"long time to complete the request. This code indicates that the server has received and "
                          @"is processing the request, but no response is available yet. This prevents the client "
        ...
        case HTTPCode200OK:
            description = @"Standard response for successful HTTP requests. The actual response will depend on the "
        ...
        case HTTPCode203NonAuthoritativeInformation:
            description = @"The server successfully processed the request, but is returning information that may be "
                          @"from another source.";
            break;

        case HTTPCode204NoContent:
            description = @"The server successfully processed the request, but is not returning any content. Usually "
                          @"used as a response to a successful delete request.";
            break;

        case HTTPCode205ResetContent:
            description = @"The server successfully processed the request, but is not returning any content. Unlike a "
                          @"204 response, this response requires that the requester reset the document view.";
            break;

        case HTTPCode206PartialContent:
            description = @"The server is delivering only part of the resource due to a range header sent by the "
                          @"client. The range header is used by tools like wget to enable resuming of interrupted "
        ...
        case HTTPCode208AlreadyReported:
            description = @"The members of a DAV binding have already been enumerated in a previous reply to this "
        ...
        case HTTPCode300MultipleChoices:
            description = @"Indicates multiple options for the resource that the client may follow. It, for instance, "
        ...
        case HTTPCode302Found:
            description = @"This is an example of industry practice contradicting the standard. The HTTP/1.0 "
                          @"specification (RFC 1945) required the client to perform a temporary redirect (the "
                          @"original describing phrase was \"Moved Temporarily\"), but popular browsers implemented "
                          @"302 with the functionality of a 303 See Other. Therefore, HTTP/1.1 added status codes 303 "
                          @"and 307 to distinguish between the two behaviours. However, some Web applications and "
                          @"frameworks use the 302 status code as if it were the 303.";
            break;

        case HTTPCode303SeeOther:
            description = @"The response to the request can be found under another URI using a GET method. When "
                          @"received in response to a POST (or PUT/DELETE), it should be assumed that the server has "
                          @"received the data and the redirect should be issued with a separate GET message.";
            break;

        case HTTPCode304NotModified:
            description = @"Indicates that the resource has not been modified since the version specified by the "
                          @"request headers If-Modified-Since or If-Match. This means that there is no need to "
                          @"retransmit the resource, since the client still has a previously-downloaded copy.";
            break;

        case HTTPCode305UseProxy:
            description = @"The requested resource is only available through a proxy, whose address is provided in the "
                          @"response. Many HTTP clients (such as Mozilla and Internet Explorer) do not correctly "
        ...
        case HTTPCode307TemporaryRedirect:
            description = @"In this case, the request should be repeated with another URI; however, future requests "
        ...
        case HTTPCode308PermanentRedirect:
            description = @"The request, and all future requests should be repeated using another URI. 307 and 308 (as "
                          @"proposed) parallel the behaviours of 302 and 301, but do not allow the HTTP method to "
        ...
        case HTTPCode401Unauthorised:
            description = @"Similar to 403 Forbidden, but specifically for use when authentication is required and has "
        ...
        case HTTPCode402PaymentRequired:
            description = @"Reserved for future use. The original intention was that this code might be used as part "
        ...
        case HTTPCode403Forbidden:
            description = @"The request was a valid request, but the server is refusing to respond to it. Unlike a 401 "
                          @"Unauthorized response, authenticating will make no difference. On servers where "
        ...
        case HTTPCode409Conflict:
            description = @"Indicates that the request could not be processed because of conflict in the request, such "
                          @"as an edit conflict in the case of multiple updates.";
            break;

        case HTTPCode410Gone:
            description = @"Indicates that the resource requested is no longer available and will not be available "
                          ...
                          @"request the resource again in the future. Clients such as search engines should remove "
                          @"the resource from their indices. Most use cases do not require clients and search engines "
        ...
        case HTTPCode414RequestURITooLong:
            description = @"The URI provided was too long for the server to process. Often the result of too much data "
                          @"being encoded as a query-string of a GET request, in which case it should be converted to "
                          @"a POST request.";
            break;

        case HTTPCode415UnsupportedMediaType:
            description = @"The request entity has a media type which the server or resource does not support. For "
        ...
        case HTTPCode418IamATeapot:
            description = @"This code was defined in 1998 as one of the traditional IETF April Fools' jokes, in RFC "
        ...
        case HTTPCode419AuthenticationTimeout:
            description = @"Not a part of the HTTP standard, 419 Authentication Timeout denotes that previously valid "
                          @"authentication has expired. It is used as an alternative to 401 Unauthorized in order to "
                          @"differentiate from otherwise authenticated clients being denied access to specific server "
                          @"resources";
            break;

        case HTTPCode420MethodFailureSpringFramework:
            description = @"Not part of the HTTP standard, but defined by Spring in the HttpStatus class to be used "
                          @"when a method failed. Or this could be returned by the Twitter Search and Trends API when "
                          @"the client is being rate limited. Other services may wish to implement the 429 Too Many "
        ...
        case HTTPCode428PreconditionRequired:
            description = @"The origin server requires the request to be conditional. Intended to prevent \"the 'lost "
        ...
        case HTTPCode429TooManyRequests:
            description = @"The user has sent too many requests in a given amount of time. Intended for use with rate "
                          @"limiting schemes.";
            break;
        ...
        case HTTPCode444NoResponseNginx:
            description = @"Used in Nginx logs to indicate that the server has returned no information to the client "
        ...
        case HTTPCode450BlockedByWindowsParentalControls:
            description = @"A Microsoft extension. This error is given when Windows Parental Controls are turned on "
        ...
        case HTTPCode451RedirectMicrosoft:
            description = @"Used in Exchange ActiveSync if there either is a more efficient server to use or the "
                          @"server can't access the users' mailbox. The client is supposed to re-run the HTTP "
                          @"Autodiscovery protocol to find a better suited server. Or unavailable for legal reasons - "
                          @"Defined in the internet draft \"A New HTTP Status Code for Legally-restricted "
                          @"Resources\". intended to be used when resource access is denied for legal reasons, e.g. "
        ...
        case HTTPCode495CertErrorNginx:
            description = @"Nginx internal code used when SSL client certificate error occurred to distinguish it from "
        ...
        case HTTPCode497HTTPToHTTPSNginx:
            description = @"Nginx internal code used for the plain HTTP requests that are sent to HTTPS port to "
        ...
        case HTTPCode499ClientClosedRequestNginx:
            description = @"Used in Nginx logs to indicate when the connection has been closed by client while the "
                          @"server is still processing its request, making server unable to send a status code back. "
                          @"Or returned by ArcGIS for Server. A code of 499 indicates that a token is required (if no "
                          @"token was submitted).";
            break;
        ...
        case HTTPCode501NotImplemented:
            description = @"The server either does not recognize the request method, or it lacks the ability to "
                          @"fulfill the request. Usually this implies future availability (e.g., a new feature of a "
                          @"web-service API).";
            break;

        case HTTPCode502BadGateway:
            description = @"The server was acting as a gateway or proxy and received an invalid response from the "
                          @"upstream server.";
            break;
        ...
        case HTTPCode504GatewayTimeout:
            description = @"The server was acting as a gateway or proxy and did not receive a timely response from the "
                          @"upstream server.";
            break;
        ...
        case HTTPCode508LoopDetected:
            description = @"The server detected an infinite loop while processing the request (sent in lieu of 208 Not "
                          @"Reported).";
            break;
        ...
        case HTTPCode598NetworkReadTimeoutErrorUnknown:
            description = @"This status code is not specified in any RFCs, but is used by Microsoft HTTP proxies to "
